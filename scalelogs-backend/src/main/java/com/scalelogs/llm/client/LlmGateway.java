package com.scalelogs.llm.client;
 ...
 * <ul>
 *   <li>{@code scalelogs.llm.provider} — chat/generation: groq (default —
 *       14,400 RPD free vs Gemini's 20) or gemini;</li>
 *   <li>{@code scalelogs.llm.embedding-provider} — embeddings: gemini (default)
...
public class LlmGateway {
    ...
    public LlmGateway(GeminiClient gemini,
                      GroqClient groq,
                      @Value("${scalelogs.llm.provider:groq}") String chatProvider,
                      @Value("${scalelogs.llm.embedding-provider:gemini}") String embeddingProvider) {
        this.gemini = gemini;
        this.groq = groq;
        ...
        if (embedGroq) {
            log.warn("scalelogs.llm.embedding-provider=groq, but Groq serves no embedding "
