package com.scalelogs.llm.config;

import com.scalelogs.llm.client.GeminiConfig;
import com.scalelogs.llm.client.GroqConfig;
 ...
 * ({@code scalelogs.llm.provider}), so Kafka lane count, pacing interval, and the
 * keys handed to {@link com.scalelogs.llm.client.LlmGateway} always agree:
 *
...
public class ApiKeyManagerConfig {
    ...
    @Value("${scalelogs.llm.provider:groq}")
    private String provider;

    @Value("${scalelogs.gemini.rate-limit-per-min:10}")
    private int geminiRateLimitPerMin;

    @Value("${scalelogs.llm.rate-limit-utilization:0.80}")
    ...
    public ApiKeyManager apiKeyManager() {
        ...
        if (apiKeys.isEmpty()) {
            throw new IllegalStateException("No " + (groq ? "Groq" : "Gemini")
                ...
                + " (scalelogs.llm.provider=" + provider + ").");
        }
