 ...
 * <ul>
 *   <li>{@code loglens.llm.provider} — chat/generation: groq (default —
 *       14,400 RPD free vs Gemini's 20) or gemini;</li>
 *   <li>{@code loglens.llm.embedding-provider} — embeddings: gemini (default)
...
public class LlmGateway {
    ...
    public LlmGateway(GeminiClient gemini,
                      GroqClient groq,
                      @Value("${loglens.llm.provider:groq}") String chatProvider,
                      @Value("${loglens.llm.embedding-provider:gemini}") String embeddingProvider) {
        this.gemini = gemini;
        this.groq = groq;
        ...
        if (embedGroq) {
            log.warn("loglens.llm.embedding-provider=groq, but Groq serves no embedding "
