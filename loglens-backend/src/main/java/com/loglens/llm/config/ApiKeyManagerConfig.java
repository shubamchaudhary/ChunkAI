package com.loglens.llm.config;

import com.loglens.llm.client.GeminiConfig;
import com.loglens.llm.client.GroqConfig;
 ...
 * ({@code loglens.llm.provider}), so Kafka lane count, pacing interval, and the
 * keys handed to {@link com.loglens.llm.client.LlmGateway} always agree:
 *
...
public class ApiKeyManagerConfig {
    ...
    @Value("${loglens.llm.provider:groq}")
    private String provider;

    @Value("${loglens.gemini.rate-limit-per-min:10}")
    private int geminiRateLimitPerMin;

    @Value("${loglens.llm.rate-limit-utilization:0.80}")
